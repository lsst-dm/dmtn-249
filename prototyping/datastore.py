...
from .primitives import OpaqueTableBatch, OpaqueTableKeyBatch
...
class Datastore(ABC):
    ...
    def predict_put_many(self, refs: Iterable[DatasetRef]) -> OpaqueTableBatch:
        ...
        `put_many` if called with the given `DatasetRef` iterable, without
        actually writing any datasets to storage.
        """
        raise NotImplementedError()
    ...
    def import_(
        ...
        *,
        ...
        opaque_data, optional
            If provided, opaque table records for these datasets and Datastore
            configuration they originated from.  If this configuration is
            compatible ``self``, these records may be used instead of creating
            new ones.
        """
        raise NotImplementedError()
    ...
    ) -> tuple[OpaqueTableBatch | None, list[FileDataset]]:
        raise NotImplementedError()
